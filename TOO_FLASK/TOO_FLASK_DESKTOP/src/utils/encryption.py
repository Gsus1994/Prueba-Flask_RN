...
def decrypt_aes(data):
    try:
        # Verificar el tipo y longitud de la clave AES
        logging.debug(f"AES_KEY type: {type(Config.AES_KEY)}, length: {len(Config.AES_KEY)}")
        ...
        logging.error(f"Error en decrypt_aes: {str(e)}")
...
def encrypt_aes(data):
    try:
        ...
        logging.error(f"Error en encrypt_aes: {str(e)}")
